...
####################################################################################
    ...
    def plot_polar_beam_pattern(self, min_elevation=None):
        ...
        **Example**:
        ...
        written by A. S. Reimer, 2013-07
        """
        assert(not min_elevation or isinstance(min_elevation,(int,float))),"min_elevation must be None, int, or float."
        if not min_elevation:
            min_el = np.min(self.data['el'])
        ...
        radii = np.cos(elevations*np.pi/180.0)
        labels = [str(x) for x in elevations]
        labels[0] = ''   # blank out the 90 degree label
        max_radius = np.cos((min_elevation-10)*np.pi/180.0)  # some padding added

        #get elevation angles and azimuth angles of each beam
        rs = np.cos(self.data['el']*np.pi/180.0)
